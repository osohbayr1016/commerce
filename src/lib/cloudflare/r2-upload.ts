    ...
    if (r2Bucket && typeof r2Bucket.put === "function") {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          throw new Error(
            `R2 bucket "${bucketName}" does not exist. Create it in Cloudflare Dashboard → R2 → Create bucket, or set R2_BUCKET_NAME in .env.local to your existing bucket name.`,
          );
        }
        throw new Error(
  ...
  try {
    if (r2Bucket && typeof r2Bucket.delete === "function") {
      ...
      if (!response.ok && response.status !== 404) {
        const errorText = await response.text();
        throw new Error(
          `R2 delete failed: ${response.status} ${response.statusText} - ${errorText}`,
        );
      }
    }
  } catch (error) {
