...
function getEnvVar(key: string, fallback?: string): string {
  ...
  if (!value) {
    if (fallback) {
      console.warn(`⚠️  Environment variable ${key} is not set. Using fallback value.`);
      return fallback;
    }
    ...
    console.warn(`⚠️  Environment variable ${key} is not set. Build will continue but runtime will fail.`);
    return `MISSING_${key}`;
  }
