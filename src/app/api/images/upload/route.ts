...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error("[upload] createClient failed:", createError);
      return NextResponse.json(
        {
    ...
    for (const file of validFiles) {
      try {
        ...
        console.error("[upload] R2 upload failed:", errorMessage, error);
    ...
    console.error("[upload] Unhandled error:", errorMessage, error);
